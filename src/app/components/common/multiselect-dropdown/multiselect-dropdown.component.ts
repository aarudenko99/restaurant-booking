...
import { CreateSideDishDialogModule } from "../../dialogs/create-side-dish/create-side-dish.module";
import { CreateSideDishComponent } from "../../dialogs/create-side-dish/create-side-dish.component";
...
@Component({
	selector: "mm-multiselect-dropdown",
...
export class MultiselectDropdownComponent
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (
			this.settings.lazyLoading &&
			this.virtualScroollInit &&
			changes.data
		) {
	...
	addNewSideDish() {
		console.log("addNewSideDish", this.data);
		const dialogRef = this.dialog.open(CreateSideDishComponent, {
			width: "50%",
			data: {}
		});
					...
					try {
						if (value.length > 1) {
							...
							throw new MyException(404, {
								msg:
									"Single Selection Mode, Selected Items cannot have more than one item."
							});
						} else {
	...
	calculateDropdownDirection() {
		...
		if (this.settings.autoPosition) {
			const dropdownHeight = this.dropdownListElem.nativeElement
				.clientHeight;
